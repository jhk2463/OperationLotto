...
submitBtn.addEventListener("click", () => {
  ...
  if (correct) {
    //Animate correct answer and rerolls dice
...
function exit() {
  ...
  if (exitRoomBtn.disabled == true) {
    //Let other player know opponent has left
    socket.send(
      JSON.stringify({
        tag: "exit",
        gameId: gameId,
...
rematchBtn.addEventListener("click", () => {
  if (rematchBool == false) {
    ...
    //Second player agrees to rematch
    resetGame();
    enableButtons(exchangeBtn);
    exchangeLabel.innerHTML = "Chances: " + chances;
    popup.style.display = "none";
...
function onMessage(msg) {
  ...
  switch (data.tag) {
    ...
    case "exit":
      statusBox.innerText =
        "Opponent has exited the room. Please exit the room to continue playing!";
...
function cardClicked(src) {
  if (cardSlotIndex.includes(answerSlotIndex)) {
    //Check if current slot is for a card
...
function dieClicked(src) {
  if (dieSlotIndex.includes(answerSlotIndex)) {
    //Check if current slot is for a die
    answerSlots[answerSlotIndex].innerHTML = src.target.innerHTML;
    answerSlots[answerSlotIndex].classList.remove("answerSlot");
    answerSlots[answerSlotIndex].classList.add("deck");
    src.target.classList.remove("deck");
...
function animateCard(target) {
  ...
  setTimeout(function () {
    target.classList.remove("animateCard");
  }, 1000);
}
