...
wss.on("request", (req) => {
  ...
  const clientId =
    ...
    Math.round(Math.random() * 10) +
    Math.round(Math.random() * 10); //Create 'unique' Id for client
  ...
  switch (data.tag) {
    ...
    case "create":
      const gameId =
        Math.round(Math.random() * 10) +
        Math.round(Math.random() * 10) +
        Math.round(Math.random() * 10); //Create 'unique' Id for game
    ...
    case "opponentSelections":
      games[data.gameId].players.forEach((player) => {
        if (player.clientId != data.clientId) {
          //Let other player know what opponent got right
          clients[player.clientId].conn.send(
            JSON.stringify({
    ...
    case "resetAnswer":
      games[data.gameId].players.forEach((player) => {
        if (player.clientId != data.clientId) {
          //Let other player opponent reset answer selections
          clients[player.clientId].conn.send(
            JSON.stringify({
              tag: "resetAnswer",
            })
